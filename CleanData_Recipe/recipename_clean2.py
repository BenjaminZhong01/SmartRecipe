...
The program is used to further clean invalid data in recipe_name_clean1.csv(contains recipe name, amount, unit and ingredidents)
This program mainly changed all invalid number in the amount column into valid data pattern

This program requires to run an existed file named "recipe_name_clean1.xls"
and will eventually create a new file named "recipe_name_clean2.csv"
...
df = pd.read_csv('recipe_name_clean1.csv')
...
df_clean.to_csv('recipe_name_clean2.csv', index=False, encoding='utf-8-sig')
